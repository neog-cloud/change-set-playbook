# Prompt — Transformar `planning.md` em `specification.md`
...
Caminho de saída: docs/change-sets/cs-[ID]/specification.md
...
Leia primeiro o `README.md`, o `templates/change-set/specification.md`, o `planning.md` indicado e, se existirem, o `README.md`, `AGENTS.md`, as convenções e os documentos de contexto do projeto-alvo. Inspecione o código e a estrutura do projeto somente para confirmar contratos, padrões, caminhos e restrições existentes. Não implemente alterações.

### Objetivo

Crie ou atualize somente o arquivo `docs/change-sets/cs-[ID]/specification.md`, usando o modelo de especificação. Escreva em português do Brasil e preserve a ligação explícita com o `planning.md`.
...
1. Use o `planning.md` como fonte de intenção, contexto, decisões e limites. Não descarte uma restrição ou dúvida relevante sem registrá-la.
...
10. Não implemente código, não crie testes, não altere o `planning.md`, não atualize índices ou árvores estruturais e não faça commit.
...
O trabalho está concluído quando `specification.md`:

- traduz fielmente o problema e o resultado esperado do `planning.md`;
