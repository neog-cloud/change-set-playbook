...
- critérios de aceite;
- matriz de rastreabilidade, evidências e estratégia de validação;
...
5. Para cada comportamento incluído, crie um requisito `RQ-*` com origem e comportamento verificável. Relacione cada `RQ-*` a pelo menos um item `CI-*`, critério `CA-*` e validação `VT-*` aplicável na matriz de rastreabilidade. Use “não se aplica” somente com justificativa.
6. Defina o checklist de implementação com unidades pequenas, concretas e verificáveis. Inclua implementação, testes, documentação e validações quando aplicáveis. Mantenha todos os itens desmarcados e associe-os aos requisitos correspondentes.
7. Escreva critérios de aceite observáveis, cobrindo sucesso, falha, regressão e casos de borda relevantes. Relacione cada critério a um requisito e a uma evidência ou teste possível.
8. Descreva a estratégia de validação com comandos ou procedimentos literais, resultado esperado e cobertura. Use comandos reais do projeto quando forem conhecidos. Não execute comandos destrutivos e não declare que uma validação passou sem executá-la.
9. Mantenha o escopo implementável em uma única Change Set. Se o checklist revelar um conjunto que exige planejamento, implementação e revisão próprios, registre essa recomendação como Sub-Change Set e preserve a separação no escopo.
10. Se houver dúvida material no planejamento, mantenha o status como `rascunho`; caso contrário, use `pronta para revisão`. Não use `validada`: esse status só pode ser atribuído após a aprovação de uma sessão independente em `review-specification-[titulo].md`.
11. Não implemente código, não crie testes, não altere o `planning-[titulo].md`, não atualize índices ou árvores estruturais e não faça commit.
12. Use nomes de arquivos e diretórios sem espaços; quando precisar sugerir um caminho, use minúsculas e `_` no lugar de espaços.
...
- contém requisitos, checklist, critérios e validações ligados na matriz de rastreabilidade;
- contém critérios de aceite observáveis e uma estratégia de validação aplicável, com procedimento literal e resultado esperado;
